...
while [[ $ensmem -lt $end_member ]];do
  ...
  if [ ${COLD_START} -eq 1 ]; then
    ...
    #${CP} -f ${INIHOME}/${INIT_YYYYMMDDHHMM}_DA/chgresprd/gfs_bndy.tile7.`printf %03i ${BNDY_BEG}`.nc INPUT/gfs_bndy.tile7.000.nc
    ${CP} -f ${INIHOME}/${INIT_YYYYMMDDHHMM}_DA/chgresprd/gfs_bndy.tile7.`printf %03i ${BNDY_END}`.nc INPUT/gfs_bndy.tile7.001.nc
  ...
  sed 's/_RESTART_INTV_/1/g'        | \
  sed 's/_FCSTLEN_/'${FCST_LENGTH}'/g' > ./model_configure
  ...
  echo "running ${exec_create_restart} for $ensmem ...."
